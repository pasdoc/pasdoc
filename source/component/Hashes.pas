...
begin
  FeldMaxValue:=7; //irgendein kleiner Wert der Eigenschaft 2**n-1
  FeldBelegt:=0;
  FMaxCapacity:=-1;
...
begin
  ...
  while Assigned(he) do begin
    if he^.Hash=h then
      if he^.key=_key then begin //gefunden
        Result:=True;
        Exit;
      end;
    he:=he^.next;
...
begin
  ...
  //neu dazureservierten Speicher leeren
  FillChar(Feld^[oldsize],(newsize-oldsize)*sizeof(PHashEntry),0);

  //dann Daten neu einordnen
...
begin
  ...
  //nichts gefunden, key muá neu angelegt werden
