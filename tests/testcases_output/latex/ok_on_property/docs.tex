...
\newlength{\tmplength}
\chapter{Unit ok{\_}on{\_}property}
\label{ok_on_property}
\index{ok{\_}on{\_}property}
\section{Description}
Test that "on" can be used as a property name, Delphi allows this.
\section{Overview}
\begin{description}
...
\label{ok_on_property.TMyClass-on}
\index{on}
\item[\textbf{on}\hfill]
\ifpdf
\begin{flushleft}
\fi
\begin{ttfamily}
public property on: Boolean read GetOn write SetOn;\end{ttfamily}
