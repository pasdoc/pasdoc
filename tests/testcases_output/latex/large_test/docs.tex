...
\section{Overview}
\begin{description}
\item[\texttt{\begin{ttfamily}TMyClass\end{ttfamily} Class}] Deprecated class.
\end{description}
\begin{description}
\item[\texttt{MyProc}]
\end{description}
...
\item[\texttt{Cdecl}]
\item[\texttt{Foo}]
\item[\texttt{Foo1}]Some other test for THTMLDocGenerator.WriteCodeWithLinks, while I'm at it:
...
\item[\texttt{\begin{ttfamily}TMyClass\end{ttfamily} Class}]These are some tags that are not allowed to have parameters: name \begin{ttfamily}TMyClass\end{ttfamily}, inherited \begin{ttfamily}TMyClassAncestor\end{ttfamily}(\ref{ok_expanding_descriptions.TMyClassAncestor}), nil \begin{ttfamily}Nil\end{ttfamily}, true \begin{ttfamily}True\end{ttfamily}, false \begin{ttfamily}False\end{ttfamily}, classname \begin{ttfamily}TMyClass\end{ttfamily}.
\end{description}
\begin{description}
\item[\texttt{RecursiveTwoAt}]aa aaaaa aa aaa \begin{ttfamily}SHGetSpecialFolderPath(0, @Path, CSIDL{\_}APPDATA, true)\end{ttfamily} aaaa aaaaaa aaaaaa aaaaaaaaa aaaa

At some point, this test caused the bug: final {$<$}/code{$>$} tag was inserted in converted form (processed with ConvertString) into html output.
...
\item[\texttt{TestLongCode}]Note that inside @longcode below I should be able to write singe @ char to get it in the output, no need to double it (like @@).
\item[\texttt{TestPasMethodTags}]This is a test of tags expanded by TPasRoutine handlers.
\item[\texttt{TestRecursiveTag}]@code and @returns (and some others) tags are recursive, you can freely put other tags inside.
\item[\texttt{TwoAt}]Write two at chars, like this @@, to get one @ in output.
\end{description}
\section{Classes, Interfaces, Objects and Records}
\ifpdf
...
\item[\texttt{TestProcExperimental}]
\item[\texttt{TestProcPlatform}]
\item[\texttt{TestProcUnimplemented}]
\end{description}
\section{Classes, Interfaces, Objects and Records}
\ifpdf
...
\fi
\begin{ttfamily}
public property TestPropertyCombined: Integer read Field; library deprecated platform;\end{ttfamily}
...
\fi
\begin{ttfamily}
public property TestPropertyCombined2: Integer read Field; library; deprecated; platform;\end{ttfamily}
...
\end{list}
\ifpdf
\subsection*{\large{\textbf{TestProcExperimental}}\normalsize\hspace{1ex}\hrulefill}
\else
\subsection*{TestProcExperimental}
\fi
\label{ok_hint_directives-TestProcExperimental}
\index{TestProcExperimental}
\begin{list}{}{
\settowidth{\tmplength}{\textbf{Description}}
\setlength{\itemindent}{0cm}
\setlength{\listparindent}{0cm}
...
\fi
\begin{ttfamily}
procedure TestProcExperimental; experimental;\end{ttfamily}
...
\end{list}
\ifpdf
\subsection*{\large{\textbf{TestProcUnimplemented}}\normalsize\hspace{1ex}\hrulefill}
\else
\subsection*{TestProcUnimplemented}
\fi
\label{ok_hint_directives-TestProcUnimplemented}
\index{TestProcUnimplemented}
\begin{list}{}{
\settowidth{\tmplength}{\textbf{Description}}
\setlength{\itemindent}{0cm}
\setlength{\listparindent}{0cm}
...
\fi
\begin{ttfamily}
procedure TestProcUnimplemented; unimplemented;\end{ttfamily}
...
\section{Overview}
\begin{description}
\item[\texttt{TestLineBreak}]1st paragraph.
\end{description}
\section{Functions and Procedures}
\ifpdf
...
\end{description}
\begin{description}
\item[\texttt{MyProc}]Link to self : \begin{ttfamily}MyProc\end{ttfamily}(\ref{ok_links-MyProc}), and a second one: \begin{ttfamily}ok{\_}links.MyProc\end{ttfamily}(\ref{ok_links-MyProc}), link to MyProc in other unit: \begin{ttfamily}ok{\_}links{\_}2.MyProc\end{ttfamily}(\ref{ok_links_2-MyProc}).
\end{description}
\section{Classes, Interfaces, Objects and Records}
\ifpdf
...
\section{Overview}
\begin{description}
\item[\texttt{\begin{ttfamily}TBar\end{ttfamily} Class}]These are links from class' description to an identifiers inside the same class.
\end{description}
\begin{description}
\item[\texttt{Foo}]
\item[\texttt{Xyz}]
...
\section{Overview}
\begin{description}
\item[\texttt{\begin{ttfamily}TTestingClass\end{ttfamily} Class}]I'm a testing class, oh ! And \begin{ttfamily}don't forget to look at my method !\end{ttfamily}(\ref{ok_link_explicite_name.TTestingClass-MyMethod})
\end{description}
\begin{description}
\item[\texttt{MyProc}]Some testing proc.
\end{description}
\section{Classes, Interfaces, Objects and Records}
\ifpdf
...
\item[\texttt{\begin{ttfamily}TMyRecord2\end{ttfamily} Record}]This record has a description, but still LaTeX output will not list it in it's summary list (in the "Overview" section).
...
\section{Overview}
\begin{description}
\item[\texttt{Foo}]This is the 1st paragraph.
\end{description}
\section{Functions and Procedures}
\ifpdf
\subsection*{\large{\textbf{Foo}}\normalsize\hspace{1ex}\hrulefill}
...
\end{description}
\begin{description}
\item[\texttt{SomeProc}]Note that this rule allows you to not specify () for *any* tag that has parameters.
\end{description}
\section{Classes, Interfaces, Objects and Records}
\ifpdf
\subsection*{\large{\textbf{EFoo Class}}\normalsize\hspace{1ex}\hrulefill}
...
\section{Overview}
\begin{description}
\item[\texttt{\begin{ttfamily}TAbstractTerminationClass\end{ttfamily} Class}](This abstract tag lacks the closing parenthesis.
\end{description}
\section{Classes, Interfaces, Objects and Records}
\ifpdf
...
\section{Overview}
\begin{description}
\item[\texttt{TestWarnings}]pasdoc should complain (display warnings) about wrong tags : @firstwarning(blah blah), \begin{ttfamily}Wrong tag inside a @code: @secondwarning(ble ble)\end{ttfamily}.
\end{description}
\section{Functions and Procedures}
\ifpdf
...
\section{Overview}
\begin{description}
\item[\texttt{Foo}]Some tags are not allowed to have parameters.
\end{description}
\section{Functions and Procedures}
\ifpdf
\subsection*{\large{\textbf{Foo}}\normalsize\hspace{1ex}\hrulefill}
