...
% Last modification Apr 20, 2000
%
% STILL TO DOCUMENT:
% -R, --description        read description from this file
% -C, --content            Read Contents for HtmlHelp from file
% -F, --footer             include file as footer
% -H, --header             include file as header
...
% --graphviz-uses          write a GVUses.gviz file that can be used for the 
%                          `dot` program from GraphViz to generate a unit 
%                          dependency graph
% --graphviz-classes       write a GVClasses.gviz file that can be used for the 
%                          `dot` program from GraphViz to generate a class 
%                          hierarchy graph
% --abbreviations          abbreviation file, format is "[name]  value", value 
%                          is trimmed, lines that do not start with '[' (or 
%                          whitespace before that) are ignored
% --aspell                 enable aspell, giving language as parameter, 
%                          currently only done in HTML output
% --ignore-words           When spell-checking, ignore the words in that file. 
%                          The file should contain one word on every line, no 
%                          comments allowed
...
{ @name is a method of @classname which overrides the method of @inherited to do 
...
without any conversion. There is no syntax checking on the
validity of the HTML syntax.

This should not be used if \LaTeX support is used, as it
will confuse \LaTeX. 
...
HTML files are usually displayed in a web browser, available on
...
A single output file {\tt docs.tex} will be created. 
With {\tt latex docs.tex} you will be able to create a dvi file
that can then be converted to a Postscript file using 
{\tt dvips docs.dvi}. Or you can also directly generate a huge
HTML file by using {\tt htlatex docs.tex}, or a PDF file
by using {\tt pdflatex docs.tex}.

It is to note that the output generated by pasdoc has been optimized for 
{\tt pdflatex} and {\tt htlatex}.
...
A single output file {\tt docs.tex} will be created, and can then
be converted by using {\tt latex2rtf docs.tex}. 
...
\subsubsection{No headers and footers in \LaTeX}

{\tt -j}\\
{\tt --notexheaders}\\

This switch will keep pasdoc from writing out standard declarations like
{\tt begin\{document\}} when writing output for \LaTeX.
This way, you will be able to include pasdoc's output by simply stating
...
A typical GNU switch, this makes pasdoc print program name and version in
one row (e.g. {\tt pasdoc 0.6.2}) to standard output.
